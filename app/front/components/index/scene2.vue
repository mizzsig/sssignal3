<template>
    <div>
        ...
        <div class="keyboard-container" style="margin-bottom: -66px; margin-left: 13px;">
            <div v-for="(value, index) in this.keyboardsBlack" v-bind:key="index" @mousedown="playPiano(index, 'black')" :style="isBlackShow(index)" class="keyboard black">
            </div>
        </div>
        <div class="keyboard-container">
            <div v-for="(value, index) in this.keyboardsWhite" v-bind:key="index" @mousedown="playPiano(index, 'white')" class="keyboard white">
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        // 画面サイズに応じた鍵盤を表示する
        onResize() {
            const countWhite = window.innerWidth > 22 ? 22 : parseInt(window.innerWidth / 26);
        ...
        playPiano(index, color) {
            console.log('playPiano', index, color);
        },
